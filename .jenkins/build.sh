...
if [[ "${JOB_BASE_NAME}" == *worker_* ]]; then
  ...
  # then we remove them
  for filename in $(find docs/recipes -name '*.html'); do
    file_basename=$(basename $filename .html)
    if [[ ! " ${FILES_TO_RUN[@]} " =~ " ${file_basename} " ]]; then
      rm $filename
    fi
  done
  for filename in $(find docs/recipes -name '*.rst'); do
  ...
  for filename in $(find docs/_sources/recipes -name '*.rst.txt'); do
    file_basename=$(basename $filename .rst.txt)
    if [[ ! " ${FILES_TO_RUN[@]} " =~ " ${file_basename} " ]]; then
      rm $filename
    fi
  done
  for filename in $(find docs/.doctrees/recipes -name '*.doctree'); do
